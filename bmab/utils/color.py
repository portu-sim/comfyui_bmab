...
color_map = {color[0]: color[1] for color in named_color}
...
def hex_to_hsl(hex_color):
	...
	if delta == 0:
		...
		s = delta / (2 - max_val - min_val) if l < 0.5 else delta / (2 - (max_val + min_val))
		h = 0
		if max_val == r:
			h = (g - b) / delta % 6
		elif max_val == g:
...
def get_color_name(color):
	...
	# Find closest matching color based on combined RGB and HSL distance
	closest_index = -1
	min_distance = float("inf")
	for i, (code, name) in enumerate(named_color):
		if color == "#" + code:
			return "#" + code, name, True

		name_rgb = code[2:5]
		name_hsl = code[5:]
		ndf1 = sum((a - int(b, 16)) ** 2 for a, b in zip(rgb, name_rgb))
		ndf2 = sum((a - int(b, 16)) ** 2 for a, b in zip(hsl, name_hsl))
		ndf = ndf1 + ndf2 * 2  # Weight HSL distance more
	...
	if closest_index == -1:
		return "#000000", "Color not found in database: " + color, False
	else:
		return color, named_color[closest_index][1], False
