...
class BMABControlNet:
	...
	def apply_controlnet(self, bind: BMABBind, control_net_name, strength, start_percent, end_percent, image_file, image=None):
		control_net = self.load_controlnet(control_net_name)

		if image is None:
			print('NONE image use file.')
			output_image, output_mask = self.load_image(image_file)
			image = output_image
